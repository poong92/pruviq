...
- cron:gh-issues-autofix
  - Time: 2026-02-28 13:17 KST
  - Action: autonomous PR/issue fixes and triage (this run)
  - What I did:
    1. Synced repo: `git checkout main && git pull origin main` (confirmed). Current branch switched to `fix/issue-117-date-range-display` after creating the fix branch (see commit below).
    2. Phase 1 — Failed PRs: No failed PRs found that required code fixes (checked open PRs via `gh pr list` — statusCheckRollup conclusions `SUCCESS`).
    3. Phase 2 — Issues (P0 > P1 > P2):
       - #117 (P2): Implemented a frontend fix to prefer UI start/end filters in the Results summary when present. Changes made to:
         - src/components/ResultsPanel.tsx (added startDate/endDate props and display logic)
         - src/components/SimulatorPage.tsx (pass startDate/endDate to ResultsPanel)
         - Branch: fix/issue-117-date-range-display (created locally).
         - Commit: 73e415d (confirmed via `git rev-parse --short HEAD`).
         - Build: `npm run build` completed successfully locally. (from build output)
         - PR opened: https://github.com/poong92/pruviq/pull/121 (checks: SUCCESS) (confirmed via `gh pr view 121`).
       - #119 (P2): X-Frame-Options header missing — diagnostic: headers already present in `public/_headers` and live site returns header (verified with `curl -s -D - -o /dev/null https://pruviq.com`). I commented on the issue and closed it as resolved.
       - #116 (P2): twitter:site meta — confirmed present in src/layouts/Layout.astro and local build passed. Commented and closed the issue.
       - #114 (P2): custom SL/TP null stats — I attempted reproduction; current API returns valid statistics for the tested payloads. Commented with test evidence and closed the issue.
       - #111 (P1): /simulate/compare timeout — Attempted to reproduce; a POST with a short client timeout (15s) timed out locally during a diagnostic attempt, suggesting the endpoint can be slow. I posted a diagnostic comment and requested backend/ops investigation (can't modify backend here).
       - #115 (P2): /indicators 404 — repo uses `/builder/indicators`; logs show `/indicators` 404 in production. Commented with evidence and recommended adding a compatibility endpoint or updating external docs. (see comment on issue #115)
       - #113 (P2): macro `derivatives` null — reproduced via `curl https://api.pruviq.com/macro` and saw `derivatives: null`. Commented with diagnosis and requested backend/ops to check upstream data & fallback handling.
    4. Phase 3 — Cleanup:
       - No stale PRs met the >3-fail threshold for auto-close this run.
    5. Updated MEMORY.md with this run summary (this entry).

  - Result / Next steps:
    - PR #121 created for #117 (frontend fix). Build and checks passed (confirmed via `gh pr view 121`).
    - Issues requiring backend/ops action and left open: #111, #115, #113 (comments added with reproductions and suggested fixes).
    - Closed issues: #119, #116, #114 as duplicate/outdated or resolved.

  - Evidence (commands / outputs):
    - Commit: `git rev-parse --short HEAD` → 73e415d (branch fix/issue-117-date-range-display)
    - PR: https://github.com/poong92/pruviq/pull/121 (created via `gh pr create` and verified with `gh pr view`)
    - Live header check: `curl -s -D - -o /dev/null https://pruviq.com` (headers include X-Frame-Options)
    - Macro endpoint reproduction: `curl -s https://api.pruviq.com/macro | jq '.derivatives'` → `null` (confirmed)
    - API simulate tests: `curl -s -X POST https://api.pruviq.com/simulate -H 'Content-Type: application/json' -d '{"sl_pct":5,"tp_pct":15}' -i` → 200 OK with `"win_rate"` present (confirmed)

  - Generated by PRUVIQ Bot (프루빅) on 2026-02-28 13:17 KST.
