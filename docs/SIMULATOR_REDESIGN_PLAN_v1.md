...
**Phase 1 "Done" =** ✅ 2026-04-21 완료
- [x] 신 UI staging 접근 가능 (`/simulate/v1/` 선행, 이후 `/simulate/` 스왑)
- [x] Playwright 46/46 across D6/D7/Phase 2/2.5/2.6/finish-up suites
- [x] 3 뷰포트 (1280/768/375) 검증 통과
- [x] **axe 0 violations** (WCAG 2.0/2.1/2.2 A+AA, EN desktop/mobile + KO desktop)
- [x] Lighthouse desktop Perf 99 / A11y 95 / BP 96 ·· mobile Perf 82 (폰트 렌더블록, 실 CDN 시 회복 기대)
- [x] EN + KO i18n 35 simV2.* 키, 완전 parity
- [x] OKXConnectCTA → `/dashboard?preset=...` 파라미터 전달
- [x] `/simulate?preset=X` legacy URL 호환

**Phase 2 "Done" =** ✅ 2026-04-21 완료
- [x] 신 UI 100% 트래픽 (`/simulate/` 및 `/ko/simulate/` 스왑)
- [x] Legacy UI `/simulate/builder/`로 보존 (strategies/[id] 딥링크 안전)
- [x] **Entry Visualizer 14+ 전략** — 7 초기 + 8 추가 (ADX·Ichimoku·PSAR·Williams·RSIBB·BBBounce·Supertrend 등)
- [x] Standard 모드 (SL/TP/TopN/Leverage/Fee/Date 슬라이더)
- [x] 라이브 성과 피드 (`/data/performance.json`)
- [x] **백테스트-vs-라이브 갭 계산** (동일 기간 재시뮬 → gap% 렌더)

**Phase 3 "Done" =** ✅ 2026-04-21 완료
- [x] `/simulate/builder/` Expert 라우트 (EN + KO)
- [x] 6 keyboard shortcuts (← → 1-7 Q S E R ?)
- [x] Shortcuts 팝오버 UI
- [x] CSV 내보내기 버튼
- [x] **A/B + funnel 이벤트 계측** (`/events` Pages Function + `emit()` 클라이언트)
- [x] 모바일 sticky CTA + preload hints

**Deferred (별도 프로젝트):**
- [ ] Legacy SimulatorPage.tsx 완전 삭제 (strategies/[id] deep-link 마이그레이션 선행 필요)
- [ ] 5 페르소나 풀 E2E 매트릭스 (현재 targeted suites만)
- [ ] Grafana Loki 이벤트 대시보드 (로그는 흐르고 있음, 뷰어만 남음)
...
| 2026-04-21 | §20 전체 체크리스트 완료 표시. Phase 1+2+3 전체 기능 구현 + 검증 완료. Deferred 항목 명시 (legacy 삭제·Grafana 대시보드 등). | 세션 종결 |
